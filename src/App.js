...
const random2 = Math.floor(Math.random() * imagesList.length)
console.log(random2)
...
class App extends Component {
  ...
  componentDidMount() {
    ...
    this.intervalId = setInterval(this.updateCounter, 1000) // 1000 milliseconds (1 second)
  }
  ...
  updateCounter = () => {
    ...
    // Check if the counter has reached 0, and if so, clear the interval.
    if (counter === 1) {
      clearInterval(this.intervalId)
    }
  }
  ...
  getFiltered = () => {
    ...
    const filtered = imagesList.filter(each => each.category === TabId)
    return filtered
  }
  ...
  checkId = id => {
    ...
    if (id === imagesList[random].id) {
      ...
      const random1 = Math.floor(Math.random() * imagesList.length)

      this.setState({random: random1, Id: imagesList[random1].id})
    }
  }
  ...
  render() {
    const {random, counter, score} = this.state
        ...
        {counter === 0 ? (
          ...
          <div className="main">
            ...
            <div className='random-image'>
              <img className='random-image-select' src={imagesList[random].imageUrl} />
              <div className='ul'>
              {tabsList.map(each => (
                <TabsList each={each} updateId={this.onUpdate} />
              ))}
              </div>
            </div>
            ...
            <div className='all-images'>
              {filtered.map(each => (
                <Item each={each} imageId={this.checkId} />
              ))}
            </div>
          </div>
        )}
